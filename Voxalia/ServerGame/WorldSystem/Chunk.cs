...
namespace Voxalia.ServerGame.WorldSystem
{
    public class Chunk
    {
        ...
        {
            int cs = CSize / lod;
            return z * CSize * CSize * lod + y * CSize * lod + x * lod;
        }
            ...
            for (int x = 0; x < CSize; x++)
            {
                for (int y = 0; y < CSize; y++)
                {
                    for (int z = 0; z < CSize; z++)
                    {
                        ...
                        {
                            BlockInternal zp = z + 1 < CSize ? GetBlockAt(x, y, z + 1) : OwningWorld.GetBlockInternal_NoLoad(Location.FromBVector(ppos) + new Location(x, y, 30));
                            BlockInternal zm = z > 0 ? GetBlockAt(x, y, z - 1) : OwningWorld.GetBlockInternal_NoLoad(Location.FromBVector(ppos) + new Location(x, y, -1));
                            BlockInternal yp = y + 1 < CSize ? GetBlockAt(x, y + 1, z) : OwningWorld.GetBlockInternal_NoLoad(Location.FromBVector(ppos) + new Location(x, 30, z));
                            BlockInternal ym = y > 0 ? GetBlockAt(x, y - 1, z) : OwningWorld.GetBlockInternal_NoLoad(Location.FromBVector(ppos) + new Location(x, -1, z));
                            BlockInternal xp = x + 1 < CSize ? GetBlockAt(x + 1, y, z) : OwningWorld.GetBlockInternal_NoLoad(Location.FromBVector(ppos) + new Location(30, y, z));
