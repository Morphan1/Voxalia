...
namespace Voxalia.ClientGame.UISystem.MenuSystem
{
    public class UITextBox : UIMenuItem
    {
        ...
        public UITextBox(string def, string info, Func<int> x, Func<int> y, int width, FontSet fonts)
        {
            Text = def;
            Info = info;
            X = x;
        ...
        public override void Render(double delta, int xoff, int yoff)
        {
            ...
            Menus.TheClient.Rendering.RenderRectangle(x - 1, y - 1, x + Width + 1, y + Fonts.font_default.Height + 1);
            GL.Enable(EnableCap.ScissorTest);
            GL.Scissor(x, Menus.TheClient.Window.Height - (y + (int)Fonts.font_default.Height), Width, (int)Fonts.font_default.Height);
