    ...
    class GrabPlayerCommand: AbstractPlayerCommand
    {
        ...
        public override void Execute(PlayerCommandEntry entry)
        {
            ...
            else
            {
                ...
                Location end = entry.Player.GetEyePosition() + Utilities.ForwardVector_Deg(ang.X, ang.Y) * 5;
                entry.Player.TheServer.PhysicsWorld.Remove(entry.Player.Body);
                BEPUphysics.Entities.Entity e = entry.Player.TheServer.Collision.CuboidLineEntity(new Location(0.1, 0.1, 0.1), entry.Player.GetEyePosition(), end);
